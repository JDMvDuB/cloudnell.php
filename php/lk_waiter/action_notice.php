<?php
	...
	if(isset($_POST['view_next_lknotice'])){
		...
		$query_s = $link->query("SELECT `notice`.id, `notice`.notice, `notice`.type, `notice`.date, `personal`.login FROM `notice` INNER JOIN `personal` ON `notice`.id_user = `personal`.id WHERE `id_user` = '$_SESSION[id_user]' ORDER BY `notice`.id DESC LIMIT $from_notice,20");
	...
	if(isset($_POST['get_notice_sound'])){
		...
		$query_s = $link->query("SELECT COUNT(`id`) as sound FROM `notice_sound_off` WHERE `id_user` = '$id_user' AND `type_personal` = 'personal'");
	...
	if(isset($_POST['notice_sound_off'])){
		...
		$query_i = $link->query("INSERT INTO `notice_sound_off` VALUES ('$row[id_m]','$id_user','personal')");
	...
	if(isset($_POST['notice_sound_on'])){
		...
		$query_d = $link->query("DELETE FROM `notice_sound_off` WHERE `id_user` = '$id_user' AND `type_personal` = 'personal'");
	}

	if(isset($_POST['is_type_notice'])){
		$is_type_notice = $_POST['is_type_notice'];
		if($is_type_notice=='orders_notice'){
			$query_s = $link->query("SELECT `notice_orders`.id, `orders`.id_personal,`personal`.login, `notice_orders`.notice,`notice_orders`.date_created, `desc`.title as title_desc, `notice_orders`.type as type_notice FROM `notice_orders` INNER JOIN `desc` ON `notice_orders`.id_desc = `desc`.id INNER JOIN `orders` ON `notice_orders`.id_orders = `orders`.id LEFT JOIN `personal` ON `orders`.id_personal = `personal`.id WHERE `notice_orders`.id_desc IN (SELECT `id_desc` FROM `service_desc` WHERE `id_waiter` = '$_SESSION[id_user]') ORDER BY `notice_orders`.id DESC LIMIT 20");
			...
			$query_s = $link->query("SELECT `notice`.id, `notice`.notice, `notice`.type, `notice`.date, `personal`.login FROM `notice` INNER JOIN `personal` ON `notice`.id_user = `personal`.id WHERE `id_user` = '$_SESSION[id_user]' ORDER BY `notice`.id DESC LIMIT 20");
			$notice = [];
			$i = 0;
			while($row = $query_s->fetch_assoc()){
				$notice[$i]['notice'] = '<span style="color:darkorange">'.$row['login'].'</span> '.$row['notice'];
