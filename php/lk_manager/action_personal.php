<?php
  ...
  // Интеграция telegram_id
  if(isset($_POST['telegram_id'])) {
    $id = FILTER($_POST['id']);
    $telegram_id = FILTER($_POST['telegram_id']);

    // Проверка, есть ли пользователь с таким id
    $query = $link->query("SELECT * FROM `personal` WHERE `id` = '$id' AND `id_cafe` = '$_SESSION[id_cafe]'");
    if($query->num_rows > 0) {
        // Обновление telegram_id
        $query_update = $link->query("UPDATE `personal` SET `telegram_id` = '$telegram_id' WHERE `id` = '$id'");
        if($query_update) {
        ...
        echo json_encode(['status' => 'error', 'data' => 'Пользователь не найден']);
    }
  }
?>