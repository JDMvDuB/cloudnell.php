<?php
	...
	if(isset($_POST['is_type_notice'])){
		...
		if($is_type_notice=='orders_notice'){
			...
			$query_s = $link->query("SELECT `notice`.id, `notice`.notice, `notice`.type, `notice`.date, `personal`.login FROM `notice` INNER JOIN `personal` ON `notice`.id_user = `personal`.id WHERE `notice`.id_cafe = '$_SESSION[id_cafe]' ORDER BY `notice`.id DESC LIMIT 20");
			$notice = [];
			$i = 0;
			while($row = $query_s->fetch_assoc()){
				$notice[$i]['notice'] = '<span style="color:darkorange">'.$row['login'].'</span> '.$row['notice'];
